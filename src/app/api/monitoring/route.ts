...
import { getActiveConnections, getSlowQueries } from "@/lib/queries";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheHit = cacheHitResult.rows[0] ?? {
      heap_read: 0,
      heap_hit: 0,
      ratio: 0,
    };

    const txnStats = txnStatsResult.rows[0] ?? {
